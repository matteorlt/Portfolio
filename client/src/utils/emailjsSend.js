...
export async function sendViaEmailJs(templateParams, which) {
  ...
  const templateId =
    which === 'contact'
      ...
      : EMAILJS_CONFIG.templateId || EMAILJS_CONFIG.contactTemplateId;
  if (!templateId?.trim()) {
    throw new Error(
      'Template EmailJS manquant : définissez VITE_EMAILJS_TEMPLATE_ID (et éventuellement VITE_EMAILJS_TEMPLATE_CONTACT_ID)'
    );
  }
  return emailjs.send(
