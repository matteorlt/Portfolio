...
function list(items) {
  ...
  return `<ul style="padding-left:20px;margin:8px 0;">${items.map((i) => `<li>${i}</li>`).join('')}</ul>`;
}

exports.buildQuoteNotificationEmail = function (payload) {
  ...
  const body = `
    ...
    <div>${String(message || 'Aucun message').replace(/\n/g, '<br>')}</div>
  `;
  return baseLayout('Nouvelle demande de devis', body);
};

exports.buildQuoteConfirmationEmail = function (payload) {
...
exports.buildContactNotificationEmail = function (payload) {
